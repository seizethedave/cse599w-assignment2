"""
Same as test_tvm_op.py but with division fixes and allows higher tolerance
for cross entropy.
"""
...
def test_conv2d():
    ...
    def np_conv2d(X, Filter, padding=0, stride=1):
        ...
        return np.matmul(filter_matrix, im2col_matrix).reshape(int(N),
            int(filter_outChannel), int(out_H), int(out_W))
